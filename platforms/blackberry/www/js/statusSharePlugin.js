...
var statusShare = {
    ...
    init: function() {
        ...
        $('.search-bar').hammer().on("tap", "td", function(e) {
            e.stopPropagation();
        ...
        statusShare.$.stuffListing.hammer().on("doubletap", "li", function() {
            ...
            $('#input-text').val(Text);
            setTimeout(function() {
                $('td[data-action="status-search"]').trigger('touch');
            }, 0)
        })
    },
    ...
    stringlength: function(str) {
        if (str.length == 0) {
            ...
            if(navigator.onLine) {
                statusShare.$.activityIndicator.html("Searching...")
